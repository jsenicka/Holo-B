###################################################################
# VCF HH Version - Lab Constructor beta v4.4.1 6/22/2022
...
Function cbConfigurator
{
    ...
    Param (
        ...
        [Net.IPAddress]$DhcpIPSubnet = "10.0.36.0",
        [INT]$DhcpSubnetCIDR = "24",
        [Net.IPAddress]$DhcpGateway = "10.0.36.1",
        [parameter(Mandatory=$true)]
...
# Write out canned Holo-Multi-Region deadwood file
    $replaceDNS +="(`n"
    $replaceDNS +="echo bind_address = \`"$CloudBuilderIP\`"`n"
    $replaceDNS +="echo chroot_dir = \`"/etc/maradns\`"`n"
    $replaceDNS +="echo upstream_servers = {}`n"
    $replaceDNS +="echo upstream_servers[\`".\`"]=\`"$($global:userOptions.labDNS)\`"`n"
    $replaceDNS +="echo upstream_servers[\`"20.0.10.in-addr.arpa.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"20.50.10.in-addr.arpa.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"20.60.10.in-addr.arpa.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"30.0.10.in-addr.arpa.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"30.50.10.in-addr.arpa.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"30.60.10.in-addr.arpa.\`"] = \`"127.0.0.1\`"`n"	
    $replaceDNS +="echo upstream_servers[\`"40.0.10.in-addr.arpa.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"40.50.10.in-addr.arpa.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"40.60.10.in-addr.arpa.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"holo-a.vcf.lab.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"holo-b.vcf.lab.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"holo-c.vcf.lab.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo upstream_servers[\`"$vcfDomainName.\`"] = \`"127.0.0.1\`"`n"
    $replaceDNS +="echo recursive_acl = \`"$($nsxSuperNet),$CloudBuilderIPSubnet/$CloudBuilderCIDR,$($avnNets[0]),$($avnNets[1]),$($tzEgress),$($tzIngress)\`"`n"
    $replaceDNS +="echo filter_rfc1918 = 0`n"
    $replaceDNS +=")> /etc/dwood3rc-holo-multi-region`n"
	$replaceDNS +="chmod 644 /etc/dwood3rc-holo-multi-region`n"
#GOBGP config
    $replaceDNS +="(`n"
    $replaceDNS +="echo [Unit]`n"
...
function setFormControls ($formway)
{
    ...
    switch ($formway)
    {
        ...
        'internalsvcs' {
                        ...
                        $lblBringUpFile.visible = $true
                        $txtBringUpFile.visible = $true
                        $txtBringUpFile.Text = "$scriptDir\Holo-B-NOLIC-44-TMM-vcf-ems-public.json"
...
foreach ($hostVM in $hostsToBuild) {
	...
	$hostVMName = "$vmPrefix$($hostVM.name)"
